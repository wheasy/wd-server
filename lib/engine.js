...
// var reg_block = /<(block\:([a-z,A-Z,0-9,\-,\_,\/]+))([^>]*)(>([^<block]*)<\/block>|\/>)/gi
...
art.helper('stringify', function (obj) {
...
function getPageInfo(filePath, options, cfg, fileData) {
    ...
    pageObj.tpl = art.render(processImageAbsPath(pageObj.code, cfg), cfg);
...
function preCompile(code, cfg) {
    ...
    while (reg_block.test(code)) {
        ...
        code = code.replace(reg_block,
            function (fullCode, modstr, modName, attrs, modcontent) {
                var retStr = '';
                var modCode;
